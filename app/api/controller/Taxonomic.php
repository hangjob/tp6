...
class Taxonomic extends BaseController
{
    ...
    public function items($id){
        ...
        $data['items'] = $taxonomic->where('parentid',$id)->limit(20)->field('id,parentid,name')->with(['navtag'=>function($query){
            $query->withLimit(5)->order('id desc')->field('id,describe,it_name,pic,keywords,create_time,parentid,uid,icon,author');
        }])->select();
